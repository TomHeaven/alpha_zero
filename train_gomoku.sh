...
nohup python3 -m training_gomoku --num_actors=64 --board_size=${CHECKBOARD_SIZE} --num_stack=8 --num_res_blocks=10 --num_filters=64 --num_fc_units=96 --num_simulations=400 --replay_capacity=30000000 --ckpt_dir=./checkpoints/gomoku/${CHECKBOARD_SIZE}x${CHECKBOARD_SIZE} --logs_dir=./logs/gomoku/${CHECKBOARD_SIZE}x${CHECKBOARD_SIZE} --save_sgf_dir=./selfplay_games/gomoku/${CHECKBOARD_SIZE}x${CHECKBOARD_SIZE} > train_gomoku_${CHECKBOARD_SIZE}x${CHECKBOARD_SIZE}.log &